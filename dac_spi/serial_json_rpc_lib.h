...
public:
  ...
  // helpers
  static size_t json_array_to_byte_array(const String& raw_json, uint8_t* array, size_t array_size);
  static size_t json_array_to_i16_array(const String& raw_json, int16_t* array, size_t array_size);
...
private:
  ...
  static const int _JSON_RPC_BUFFER_SIZE = 100000;
...
size_t SerialJsonRpcBoard::json_array_to_byte_array(const String& raw_json, uint8_t* array, size_t array_size) {
  ...
  for (size_t i = 0; i < json_array.size(); i++) {
    array[i] = json_array[i].as<uint8_t>();
  }
  return json_array.size();
}

size_t SerialJsonRpcBoard::json_array_to_i16_array(const String& raw_json, int16_t* array, size_t array_size) {
  DynamicJsonDocument json_doc(raw_json.length());
  deserializeJson(json_doc, raw_json);
  JsonArray json_array = json_doc.as<JsonArray>();
  if (json_array.size() > array_size) {
  ...
  for (size_t i = 0; i < json_array.size(); i++) {
    array[i] = json_array[i].as<int16_t>();
  }
  return json_array.size();
}
